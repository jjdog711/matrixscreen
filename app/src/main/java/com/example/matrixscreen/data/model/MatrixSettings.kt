    ...
    fun update(updates: Map<String, Any>): MatrixSettings {
        ...
        updates.forEach { (key, value) ->
            updated = when (key) {
                ...
                "advancedColorsEnabled" -> updated.copy(advancedColorsEnabled = value as Boolean)
                "linkUiAndRainColors" -> updated.copy(linkUiAndRainColors = value as Boolean)
                "fontSize" -> updated.copy(fontSize = value as Int)
                "symbolSetId" -> updated.copy(symbolSetId = value as String)
                "savedCustomSets" -> updated.copy(savedCustomSets = value as List<CustomSymbolSet>)
                "activeCustomSetId" -> updated.copy(activeCustomSetId = value as String?)
                "maxTrailLength" -> updated.copy(maxTrailLength = value as Int)
                "maxBrightTrailLength" -> updated.copy(maxBrightTrailLength = value as Int)
                "themePresetId" -> updated.copy(themePresetId = value as String?)
